...
DOMAIN_ONLY_FEATURE_POLICY = """# Feature Search Policy
...
def main() -> None:
    ...
    parser.add_argument(
        "--domain-only",
        action="store_true",
        help="Do not provide any tree-model guidance or tree-inspired hints; rely only on base40 SVM behavior and domain reasoning.",
    )
    ...
    context_md = build_context_markdown(
        ...
        include_tree_hints=not args.domain_only,
    )
    ...
    if not policy_path.exists():
        initial_policy = DOMAIN_ONLY_FEATURE_POLICY if args.domain_only else DEFAULT_FEATURE_POLICY
        policy_path.write_text(initial_policy + "\n", encoding="utf-8")
    ...
    def persist_state() -> None:
        ...
        summary = build_summary_payload(
            ...
            config={
                ...
                "random_state": args.random_state,
                "svm_c": args.svm_c,
                "tree_guidance_path": str(tree_guidance_path) if tree_guidance_path is not None else "",
                "domain_only": bool(args.domain_only),
                "llm_model": llm_model,
