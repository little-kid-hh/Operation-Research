...
public class MILP_Loading_6orientations {
    private static double timeLimitSeconds() {
        return Double.parseDouble(System.getProperty("or2023.bpp.timeLimit6ori", "300.0"));
    }
...
//                    }
            ...
            throw new RuntimeException("Gurobi failed while solving 6-orientation MILP", e);
        } catch (Exception e) {
            System.err.println("Exception during optimization");
            throw new RuntimeException("Failed while solving 6-orientation MILP", e);
        } finally {
            if (model != null) {
                model.dispose();
            }
