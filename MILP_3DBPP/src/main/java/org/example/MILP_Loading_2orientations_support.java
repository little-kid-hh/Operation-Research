...
public class MILP_Loading_2orientations_support {
    public static int MILP_single_box(List<Double> L_Packages,
        ...
        try {
            ...
            // o_i_k 表示 i，k之间在xy投影上有重合
            // projection_x_i_k 表示ij 的在x上的重叠长度
            // projection_y_i_k 表示ij 的在x上的重叠长度
            // s_x_i_k 表示ij k给予i支撑的在x上的重叠长度
            // s_y_i_k 表示ij k给予i支撑的在y上的重叠长度
            ...
            List<List<GRBVar>> projection_xp = new ArrayList<>();
            List<List<GRBVar>> projection_yp = new ArrayList<>();
            List<List<GRBVar>> sxp = new ArrayList<>();
            List<List<GRBVar>> syp = new ArrayList<>();
            ...
            for (int i = 0; i < numParcels; ++i) {
                ...
                List<GRBVar> projection_xpRow = new ArrayList<>();
                List<GRBVar> projection_ypRow = new ArrayList<>();
                List<GRBVar> sxpRow = new ArrayList<>();
                List<GRBVar> sypRow = new ArrayList<>();
                ...
                for (int k = 0; k < numParcels; ++k) {
                    ...
                    projection_xpRow.add(model.addVar(0, L_max, 0, GRB.CONTINUOUS, "projection_xp_" + i + "_" + k));
                    projection_ypRow.add(model.addVar(0, W_max, 0, GRB.CONTINUOUS, "projection_yp_" + i + "_" + k));
                    sxpRow.add(model.addVar(0, L_max, 0, GRB.CONTINUOUS, "sxp_" + i + "_" + k));
                    sypRow.add(model.addVar(0, W_max, 0, GRB.CONTINUOUS, "syp_" + i + "_" + k));
                }
                axp.add(axpRow);
                ayp.add(aypRow);
                azp.add(azpRow);
                ...
                Bzp.add(BzpRow);
                oik.add(oikRow);
                projection_xp.add(projection_xpRow);
                projection_yp.add(projection_ypRow);
                sxp.add(sxpRow);
                syp.add(sypRow);
            }
...
// 约束 3: z.get(i) + 0.5 * (t.get(i).get(2).get(0) * parcel_p.get(i) / H_max + t.get(i).get(2).get(1) * parcel_q.get(i) / H_max + t.get(i).get(2).get(2) * parcel_r.get(i) / H_max) <= sumH
            ...
            // support area constraint
            for (int i = 0; i < numParcels; ++i) {
                for (int k = 0; k < numParcels; ++k) {
                    if (i == k) {
                        continue; //避免自我堆叠
                    }
                    // 1. 创建变量：minEndX_ik = min(xir, xkr)
                    GRBVar minEndX = model.addVar(0.0, GRB.INFINITY, 0.0, GRB.CONTINUOUS, "minEndX_" + i + "_" + k);
                    model.addGenConstrMin(minEndX, new GRBVar[]{xr.get(i), xr.get(k)}, L_max,"minEndX_" + i + "_" + k); // 用线性表达式 min

                    // 2. 创建变量：maxEndX_ik = max(xi, xk)
                    GRBVar maxEndX = model.addVar(0.0, GRB.INFINITY, 0.0, GRB.CONTINUOUS, "maxEndX_" + i + "_" + k);
                    model.addGenConstrMax(maxEndX, new GRBVar[]{x.get(i), x.get(k)}, 0.0,"maxEndX_" + i + "_" + k); // 用线性表达式 min

                    // 3. 创建变量：deltaEndX_ik= minEndX_ik - maxEndX_ik
                    GRBVar deltaEndX = model.addVar(0.0, GRB.INFINITY, 0.0, GRB.CONTINUOUS, "deltaEndX_" + i + "_" + k);
                    GRBLinExpr expr1 = new GRBLinExpr();
                    expr1.addTerm(1.0, minEndX);
                    expr1.addTerm(-1.0, maxEndX);
                    model.addConstr(deltaEndX, GRB.EQUAL, expr1, "c_32_" + i + "_" + k);



                    // 4. projection_x_ik = max(0, deltaEndX_ik)
                    model.addGenConstrMax(projection_xp.get(i).get(k), new GRBVar[]{deltaEndX}, 0.0,"projection_xp_" + i + "_" + k);

//                    // s_x_ik = min(B_ik*L_max, projection_x_ik)
//                    GRBLinExpr expr_s_x_ij_1 = new GRBLinExpr();
//                    expr_s_x_ij_1.addTerm(L_max, Bzp.get(i).get(k));
//                    GRBVar
//                    model.addGenConstrMin(sxp.get(i).get(k), )

                    // 重复一遍，创建y的
                    // 1. 创建变量：minEndY_ik = min(yir, ykr)
                    GRBVar minEndY = model.addVar(0.0, GRB.INFINITY, 0.0, GRB.CONTINUOUS, "minEndY_" + i + "_" + k);
                    model.addGenConstrMin(minEndY, new GRBVar[]{yr.get(i), yr.get(k)}, W_max,"minEndY_" + i + "_" + k); // 用线性表达式 min

                    // 2. 创建变量：maxEndY_ik = max(yi, yk)
                    GRBVar maxEndY = model.addVar(0.0, GRB.INFINITY, 0.0, GRB.CONTINUOUS, "maxEndY_" + i + "_" + k);
                    model.addGenConstrMax(maxEndY, new GRBVar[]{y.get(i), y.get(k)}, 0.0,"maxEndY_" + i + "_" + k); // 用线性表达式 min

                    // 3. 创建变量：deltaEndY_ik= minEndY_ik - maxEndY_ik
                    GRBVar deltaEndY = model.addVar(0.0, GRB.INFINITY, 0.0, GRB.CONTINUOUS, "deltaEndY_" + i + "_" + k);
                    GRBLinExpr expr2 = new GRBLinExpr();
                    expr2.addTerm(1.0, minEndY);
                    expr2.addTerm(-1.0, maxEndY);
                    model.addConstr(deltaEndY, GRB.EQUAL, expr2, "c_33_" + i + "_" + k);

                    // ∑ j∈Ib  (sx  i j + sy  ij)  )2  ≥∑  j∈Ib  (sx  i j − sy  i j )2 + 3.0*li*wi i ∈ It .
