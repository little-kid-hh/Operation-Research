...
public class MILP_Loading_2orientations_fragility_mine {
...
// 约束 3: z.get(i) + 0.5 * (t.get(i).get(2).get(0) * parcel_p.get(i) / H_max + t.get(i).get(2).get(1) * parcel_q.get(i) / H_max + t.get(i).get(2).get(2) * parcel_r.get(i) / H_max) <= sumH
            ...
            for (int i = 0; i < numParcels; ++i) {
                for (int k = 0; k < numParcels; ++k) {
                    ...
                    // 1. z[i] - zr[k]=delta[i][k]
                    GRBLinExpr expr1 = new GRBLinExpr();
                    expr1.addTerm(1.0, z.get(i)); // z.get(i)
                    expr1.addTerm(-1.0, zr.get(k)); // z.get(i)
                    model.addConstr(deltazp.get(i).get(k), GRB.EQUAL, expr1, "c_32_"+i+"_"+k);

                    // 2. uzp[i][k] = |z[i] - zr[k]| =|delta[i][k]|
                    model.addGenConstrAbs(uzp.get(i).get(k),deltazp.get(i).get(k), "c_33_"+i+"_"+k);

                    // 3. uzp[i][k] <= small_epsilon => Az_i_k =1
//                    GRBLinExpr uzExpr1 = new GRBLinExpr();
//                    uzExpr1.addTerm(1.0, uzp.get(i).get(k));
//                    uzExpr1.addConstant(-small_epsilon);
//                    uzExpr1.addTerm(H_max,Azp.get(i).get(k));
//                    model.addConstr(uzExpr1, GRB.GREATER_EQUAL, 0, "c_34_"+i+"_"+k);

                    GRBLinExpr uzExpr1 = new GRBLinExpr();
                    uzExpr1.addTerm(1.0, uzp.get(i).get(k));  // 将变量转为表达式
                    model.addGenConstrIndicator(Azp.get(i).get(k), 1, uzExpr1, GRB.LESS_EQUAL, small_epsilon, "c_34_"+i+"_"+k);

                    // 4.  uzp[i][k] >= big_epsilon => Az_i_k =0
//                    GRBLinExpr uzExpr2 = new GRBLinExpr();
//                    uzExpr2.addTerm(1.0, uzp.get(i).get(k));  // 将变量转为表达式
//                    uzExpr2.addConstant(-H_max);
//                    uzExpr2.addTerm(H_max, Azp.get(i).get(k));
//                    model.addConstr(uzExpr2, GRB.LESS_EQUAL, 0, "c_35_"+i+"_"+k);

                    GRBLinExpr uzExpr2 = new GRBLinExpr();
                    uzExpr2.addTerm(1.0, uzp.get(i).get(k));  // 将变量转为表达式
                    model.addGenConstrIndicator(Azp.get(i).get(k), 0, uzExpr2, GRB.GREATER_EQUAL, big_epsilon, "c_35_"+i+"_"+k);


                    // 5. Bz_i_k =1 => a_ik_x =0 and a_ki_x =0 and a_ik_y =0 and a_ki_y =0 and A_ik =1
                    // 首先构造一个大的and 表达式，然后用indicator约束
                    GRBVar not_a_ik_x = model.addVar(0, 1, 0, GRB.BINARY, "not_a_ik_x");
                    GRBLinExpr expra1 = new GRBLinExpr();
                    expra1.addConstant(1.0);
                    expra1.addTerm(-1.0, axp.get(i).get(k));
                    model.addConstr(not_a_ik_x, GRB.EQUAL, expra1, "def_not_a_ik_x_"+i+"_"+k);

                    GRBVar not_a_ki_x = model.addVar(0, 1, 0, GRB.BINARY, "not_a_ki_x_"+k+"_"+i);
                    GRBLinExpr expra2 = new GRBLinExpr();
                    expra2.addConstant(1.0);
                    expra2.addTerm(-1.0, axp.get(k).get(i));
                    model.addConstr(not_a_ki_x, GRB.EQUAL, expra2, "def_not_a_ki_x");

                    GRBVar not_a_ik_y = model.addVar(0, 1, 0, GRB.BINARY, "not_a_ik_y"+i+"_"+k);
                    GRBLinExpr expra3 = new GRBLinExpr();
                    expra3.addConstant(1.0);
                    expra3.addTerm(-1.0, ayp.get(i).get(k));
                    model.addConstr(not_a_ik_y, GRB.EQUAL, expra3, "def_not_a_ik_y");

                    GRBVar not_a_ki_y = model.addVar(0, 1, 0, GRB.BINARY, "not_a_ki_y"+k+"_"+i);
                    GRBLinExpr expra4 = new GRBLinExpr();
                    expra4.addConstant(1.0);
                    expra4.addTerm(-1.0, ayp.get(k).get(i));
                    model.addConstr(not_a_ki_y, GRB.EQUAL, expra4, "def_not_a_ki_y");

                    // 创建一个大and变量
                    GRBVar cond_all_true = model.addVar(0, 1, 0, GRB.BINARY, "cond_all_true"+i+"_"+k);
                    GRBVar[] and_vars = new GRBVar[]{not_a_ik_x, not_a_ki_x, not_a_ik_y, not_a_ki_y, Azp.get(i).get(k)};
                    model.addGenConstrAnd(cond_all_true, and_vars, "cond_all_true_def");

                    // 使用indicator约束
                    GRBLinExpr BzExpr = new GRBLinExpr();
                    BzExpr.addTerm(1.0, Bzp.get(i).get(k));
                    model.addGenConstrIndicator(cond_all_true,1, BzExpr, GRB.EQUAL, 1, "c_36_"+i+"_"+k);
                    ...
                    model.addConstr(parcel_fragility.get(i), GRB.GREATER_EQUAL, fragilityExpr_k, "c_37_"+i+"_"+k);
                }
            }
