...
public class MILP_Loading_2orientations_fragility {
    public static int MILP_single_box(List<Double> L_Packages,
        ...
        try {
            ...
            System.out.println(parcel_fragility);
            System.out.println(parcel_p);
            System.out.println(parcel_q);
            System.out.println(parcel_r);
            // 设置时间上限为 60 秒
            ...
            // mik that is equal to 1, if  z′  k ≥ zi and 0 otherwise.
            ...
            List<List<GRBVar>> mzp = new ArrayList<>();
            for (int i = 0; i < numParcels; ++i) {
                ...
                List<GRBVar> mzpRow = new ArrayList<>();
                ...
                for (int k = 0; k < numParcels; ++k) {
                    ...
                    mzpRow.add(model.addVar(0, 1, 0, GRB.BINARY, "mzp_" + i + "_" + k));
...
// 约束 3: z.get(i) + 0.5 * (t.get(i).get(2).get(0) * parcel_p.get(i) / H_max + t.get(i).get(2).get(1) * parcel_q.get(i) / H_max + t.get(i).get(2).get(2) * parcel_r.get(i) / H_max) <= sumH
            ...
            for (int i = 0; i < numParcels; ++i) {
                for (int k = 0; k < numParcels; ++k) {
                    ...
                    //z′  k − zi ≤ uik, ∀i, k,
                    // zi − z′  k ≤ uik, ∀i, k
                    GRBLinExpr expr_abs_1 = new GRBLinExpr();
                    expr_abs_1.addTerm(1.0, z.get(i)); // z.get(i)
                    expr_abs_1.addTerm(-1.0, zr.get(k)); // zr.get(k)
                    model.addConstr(uzp.get(i).get(k), GRB.GREATER_EQUAL, expr_abs_1, "c_32_11_"+i+"_"+k);
                    GRBLinExpr expr_abs_2 = new GRBLinExpr();
                    expr_abs_2.addTerm(1.0, zr.get(k)); // zr.get(k)
                    expr_abs_2.addTerm(-1.0, z.get(i)); // zi.get(k)
                    model.addConstr(uzp.get(i).get(k), GRB.GREATER_EQUAL, expr_abs_2, "c_32_22_"+i+"_"+k);


                    //uik ≤ z′  k − zi + 2H (1 − mik), ∀i, k,
                    // uik ≤ zi − z′  k + 2Hmik, ∀i, k,

                    GRBLinExpr expr1 = new GRBLinExpr();
                    expr1.addTerm(-1.0, z.get(i)); // z.get(i)
                    expr1.addTerm(1.0, zr.get(k)); // zr.get(k)
                    expr1.addConstant(2*H_max);
                    expr1.addTerm(-2*H_max, mzp.get(i).get(k));
                    model.addConstr(uzp.get(i).get(k), GRB.LESS_EQUAL, expr1, "c_32_"+i+"_"+k);

                    GRBLinExpr expr2 = new GRBLinExpr();
                    expr2.addTerm(1.0, z.get(i)); // z.get(i)
                    expr2.addTerm(-1.0, zr.get(k)); // zr.get(k)
                    expr2.addTerm(2*H_max, mzp.get(i).get(k));
...
//                    model.addConstr(uzp.get(i).get(k), GRB.LESS_EQUAL, expr2, "c_33_"+i+"_"+k);
                    ...
                    GRBLinExpr expr2_6 = new GRBLinExpr();
                    expr2_6.addTerm(H_max,uzp.get(i).get(k));
                    model.addConstr(expr2_5, GRB.LESS_EQUAL, expr2_6, "c_34_"+i+"_"+k);
...
//                    // 获取所有变量
                ...
                model.computeIIS();
                model.write("model_fragility.ilp");
                int status = (int) model.get(GRB.IntAttr.Status);
