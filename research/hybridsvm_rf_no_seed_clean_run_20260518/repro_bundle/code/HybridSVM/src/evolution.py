...
class Rule:
    ...
    harmed_correct: int = 0

    def compute_score(self, eps: float = 1e-6) -> float:
        """Legacy FN/FP branch-balanced F1-style score."""
    ...
    def fit(
        ...
        *,
        score_mode: str = "net_gain_v1",
        harm_weight: float = 3.0,
        score_hard_guard: bool = True,
    ) -> None:
        """
        ...
        corrected = fn_corr + fp_corr
        harmed = easy_tn_over + easy_tp_over
        self.easy_overrides = harmed
        self.corrected_errors = corrected
        self.harmed_correct = harmed
        self.fn_corr = fn_corr
        self.fp_corr = fp_corr
        self.fn_miss = fn_miss
        self.fp_over = fp_over
        if score_mode == "legacy_f1":
            penalty = 1.0 - 0.1 * harmed / max(len(easy_tn) + len(easy_tp), 1)
            penalty = max(penalty, 0.0)
            raw_score = self.compute_score()
            self.score = raw_score * penalty
        else:
            hard_total = max(len(fn_cases) + len(fp_cases), 1)
            net_gain = corrected - float(harm_weight) * harmed
            self.score = net_gain / hard_total
            # Hard guard: if a rule harms at least as many easy-correct rows as it fixes,
            # do not let it outrank abstain-style rules.
            if score_hard_guard and harmed >= corrected and harmed > 0:
                self.score = min(self.score, 0.0)
            # Explore variant: slight positive bump for rules that correct at least one hard case
            # while keeping harm bounded, to avoid getting stuck at trivial abstain.
            if (
                score_mode == "net_gain_explore_v1"
                and corrected > harmed
                and corrected > 0
            ):
                self.score += 0.001 * corrected
        self.fitness_history.append(self.score)
...
def rule_sort_key(r: Rule) -> tuple[float, float, float]:
    ...
    """
    total_corr = float(getattr(r, "corrected_errors", r.fn_corr + r.fp_corr))
    harmed = float(getattr(r, "harmed_correct", r.easy_overrides))
    return (float(r.score), total_corr, -harmed)
...
RULE_SKELETON = textwrap.dedent("""\
    ...
    Parameters:
        ...
        features : dict   — raw feature dict with all feature values; boundary-gated
                            runs may also include synthetic SVM fields
    ...
    IMPORTANT:
      ...
      - In boundary-gated Hybrid runs, features may include:
          * svm_decision_score: signed SVM margin (negative => SVM predicts 0, positive => 1)
          * svm_abs_decision_score: abs(svm_decision_score)
          * svm_near_boundary: whether the outer Hybrid gate selected this row
    ...
    def apply_rule_patch(svm_prob: float, features: dict) -> int:
        ...
        svm_margin  = features.get('svm_decision_score', None)
...
class EvolutionConfig:
    ...
    timeout_per_call: int = 180       # seconds
    ...
    score_mode: str = "net_gain_v1"  # "net_gain_v1" | "net_gain_explore_v1" | "legacy_f1"
    harm_weight: float = 3.0
    score_hard_guard: bool = True
...
class EvolutionEngine:
    ...
    def _fit_rule(self, rule: Rule, fn_cases, fp_cases, easy_tn, easy_tp) -> Rule:
        rule.fit(
            ...
            score_hard_guard=self.config.score_hard_guard,
        )
        return rule
    ...
    def _write_hybrid_iter_snapshot(
        ...
        X_test_df, y_test, svm_probs_test, dispatch_ids_test, top_k = snapshot_ctx[:5]
        decision_scores = snapshot_ctx[5] if len(snapshot_ctx) > 5 else None
        boundary_margin = snapshot_ctx[6] if len(snapshot_ctx) > 6 else None
        fn_margin = snapshot_ctx[7] if len(snapshot_ctx) > 7 else None
        fp_margin = snapshot_ctx[8] if len(snapshot_ctx) > 8 else None
...
def save_rules(rules: list[Rule], path: pathlib.Path) -> None:
    ...
    for r in rules:
        data.append({
            ...
            "harmed_correct": r.harmed_correct,
        })
...
def load_rules(path: pathlib.Path) -> list[Rule]:
    ...
    for item in data:
        r = Rule(
            ...
            corrected_errors=int(item.get("corrected_errors", item.get("fn_corr", 0) + item.get("fp_corr", 0))),
            harmed_correct=int(item.get("harmed_correct", item.get("easy_overrides", 0))),
        )
        rules.append(r)
    return rules
...
def boundary_candidate_mask(
    ...
    *,
    ...
    fn_margin: float | None = None,
    fp_margin: float | None = None,
) -> np.ndarray | None:
    """
    Return a boolean mask for rows close enough to the SVM decision boundary.

    Signed convention:
      - decision < 0: SVM predicts class 0; FN rescue rules may flip to 1.
      - decision >= 0: SVM predicts class 1; FP veto rules may flip to 0.

    If no margin is configured, return None to signal "no gate".
    """
    if boundary_margin is None and fn_margin is None and fp_margin is None:
        return None
    if decision_scores is None:
        raise ValueError("Boundary-gated rules require decision_scores from the SVM.")

    scores = np.asarray(decision_scores, dtype=float).ravel()
    if fn_margin is None:
        fn_margin = boundary_margin
    if fp_margin is None:
        fp_margin = boundary_margin

    mask = np.zeros_like(scores, dtype=bool)
    if fn_margin is not None and fn_margin >= 0:
        mask |= (scores < 0) & (scores >= -float(fn_margin))
    if fp_margin is not None and fp_margin >= 0:
        mask |= (scores >= 0) & (scores <= float(fp_margin))
    return mask
...
def apply_rules(
    ...
    When a boundary margin is configured, rules are invoked only on rows close
    to the signed SVM hyperplane. This keeps high-confidence easy rows on the
    original SVM path and makes the Hybrid layer behave like a selective second
    review instead of a global override.
    """
    ...
    for i in range(len(X_df)):
        if boundary_mask is not None and not bool(boundary_mask[i]):
            continue
        feat = X_df.iloc[i].to_dict()
        if decision_arr is not None:
            score = float(decision_arr[i])
            feat.setdefault("svm_decision_score", score)
            feat.setdefault("svm_abs_decision_score", abs(score))
            feat.setdefault("svm_near_boundary", bool(boundary_mask[i]) if boundary_mask is not None else True)
        p = svm_probs[i]
