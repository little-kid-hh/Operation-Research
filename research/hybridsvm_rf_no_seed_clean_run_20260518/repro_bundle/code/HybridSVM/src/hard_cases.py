...
def _typical_case_line(case: Case) -> str:
    margin = case.features.get("svm_decision_score", None)
    parts = [
        f"dispatch={case.dispatch_id}",
        f"prob={case.svm_prob:.4f}",
    ]
    if margin is not None:
        parts.append(f"margin={float(margin):.4f}")
    parts.extend(
        [
...
def render_hard_case_prompt(
    ...
    *,
    ...
    instruction_style: str = "conservative",
) -> str:
    """
    ...
    if include_llm_instruction:
        ...
        if instruction_style == "explore":
            instruction_text = (
                "请分析以上FN/FP错题与Easy TN/TP对照样本，找出SVM线性模型在错题上的失效规律，"
                "优先给出**窄且可解释**的覆盖条件，但允许进行小范围探索性覆盖。"
                "目标是先找到能稳定修正 FN/FP 的条件，再逐步收窄以减少误伤。"
                "对 Easy TN/TP 及 §5 中的对照行应谨慎处理，但不要求完全零触发。"
