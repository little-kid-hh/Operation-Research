...
def fit_svm_pipeline(
    ...
    *,
    ...
    """Fit a linear SVM pipeline on a caller-provided train split."""
    feature_names = list(X_train_df.columns)
    y_arr = np.asarray(y_train).ravel()

    scaler = MinMaxScaler()
    X_train_scaled = scaler.fit_transform(X_train_df[feature_names].values)
    ...
    model.fit(X_train_scaled, y_arr)
...
def train_svm(
    ...
    X_train_scaled = pipeline.scaler.transform(X_train[feature_names].values)
    X_test_scaled = pipeline.scaler.transform(X_test[feature_names].values)
